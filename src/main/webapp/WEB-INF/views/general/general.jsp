...
(function(){
		...
		({	url:$base+"/list",
			...
			{	
				...
				$.each(r.catalogo,(i,item)=>{	
					...
					generalMap[item.cat_id_n] = {id:item.cat_id_n,value:item.cat_desc_str};
				});
	...
	function loadData()
	{
		...
		({	url:$base+"/detList",
			data:{idPadre: $general.catalogoPadre},
			callback:(r)=>{
	...
	this.addGeneral =(btn)=>
	{
		$portal.system.blockButton(btn);	
		$general.catalogoPadre = $("#f_categoria").val();
		$general.catalogoPadreTitle = generalMap[$general.catalogoPadre].value;
	...
	function updateEstatus(id,estatus)
	{
		...
		({	url:$base+"/updateEstatus",
			data:{ idDetalle:id,estatus:estatus },
			callback:function(r){
				$general.refreshData();
			}
		}).always(()=>{ loading.close(); });
